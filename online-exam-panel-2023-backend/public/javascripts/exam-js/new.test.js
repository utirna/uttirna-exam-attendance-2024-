...
var Mytest = function () {
  (this.configTest = function (tbi, qts, callback) {
    ...
    (this.getQuestionView = function (data_array, index_id) {
      ...
      if (data_array.length >= index_id) {
        ...
        if (question_array.q_ask_in != "") {
          ...
          $("#i-ask-in").html("").parent().addClass("hide");
        }
    ...
    (this._printTypeFour = function (question, index_id) {
      ...
      for (var m_row = 0; m_row < mat_row_count; m_row++) {
        ...
        for (var m_col = 0; m_col < mat_col_count + 1; m_col++) {
          ...
          if (!m_col) {
            matrix_body +=
              "<td>" + String.fromCharCode(97 + parseInt(m_row)) + "</td>";
          } else {
      ...
      for (var i = 0; i < max_lenth; i++) {
        ...
        if (typeof col_1[i] == "undefined") {
          _col_tr +=
            '<td style="text-align: left !important">' + cap_a + ")&nbsp</td>";
        } else {
          _col_tr +=
      ...
      q +=
        ...
        "</div>";
      q +=
        '<div class="col-sm-10 ans-panel col-xs-offset-1 col-sm-offset-1' +
        'col-md-offset-1 col-lg-offset-1"" style="margin-top: 2%"><h5><strong>Answer</strong><h5>' +
        and_option;
    ...
    (this._printTypeThree = function (question, index_id) {
      ...
      q +=
        '<div class="col-xs-8 col-sm-8 col-md-8 col-lg-8 col-xs-offset-2 col-sm-offset-2 col-md-offset-2 col-lg-offset-2 col-xs-offset-2 " >' +
        '<div class="table-responsive">' +
        '<table class="table table table-bordered">' +
        '<tbody style="font-size: 12pt; text-align: left">' +
        "<tr>" +
        '<td width="20%">Statement 1:</td>' +
        '<td style="text-align: left !important"><strong>' +
        question.q_a +
        "</strong></td>" +
        "</tr>" +
        "<tr>" +
      ...
      q += "</div>";
      q +=
        '<div class="col-sm-12 ans-panel" style="margin-top: 2%"><h5><strong>Answer</strong><h5>';
  ...
  (this._printTypeOne = function (question, index_id) {
     ...
     */
    var q =
      '<div class="col-sm-12 question-panel">' + "<p>" + question.q + "</p>";
    if (question.q_i_q != 0) {
    ...
    (this.setOptionData = function (
      ...
      option_text = option_text.replace(/(<p[^>]+?>|<p>|<\/p>)/gim, "");
      ...
      if (option_image == "0") {
        if (option_text != "") {
          _tr =
            '<div class="radio c_stud_ans"  style="padding-bottom:1.5%" data-id="i-opt-' +
            btn_no +
            '" data-value="' +
            value +
            '">' +
            '<label style="font-size: 15px;width:100%"><input  type="radio" ' +
            _checked +
            ' name="optradio" id="i-opt-' +
            btn_no +
            '" value="' +
            value +
            '">' +
        ...
        _tr =
          '<div class="radio c_stud_ans" data-id="i-opt-' +
          btn_no +
          '" data-value="' +
          value +
          '">' +
          '<label style="font-size: 15px;width:100%"><input class="" ' +
          _checked +
          '  type="radio" name="optradio" id="i-opt-' +
          btn_no +
          '" value="' +
          value +
          '"><img src="' +
      ...
      $.ajax({
        ...
        error: function (error_data) {
          switch (error_data.status) {
            ...
            default:
              var m1 =
                "You have Network Connection Error.Please contact to Staff [ परत लॉगिन करा ]";
              redirectMessage(m1, _getUrl());
              break;
          }
        },
...
function showHideReconect(msg) {
  $(".back-out").removeClass("hide");
  $("#exam-redirect").removeClass("hide");
  $("#redirect-text").html(msg);
}
function redirectMessage(msg = "", url) {
  is_time_pause = true;
  showHideReconect(msg);
  setTimeout(function () {
    location.replace("/");
  }, 2000);
}
...
setInterval(function () {
  _tmp_ans = "b";
  if (is_time_pause === true) {
    return false;
  }

  if (current_question_index == 249) {
    alert("All Ans Done");
    //return false;
    is_time_pause = true;
    var _data = {
    ...
    mytest.saveTest(_data, function (json_data) {
      if (json_data.call) {
        ...
        retryMsg(
          "Times Up,But Unable to submit your test,Click on ok to try again.",
          function (type) {
            mytest.submitRetry(_data);
          }
        );
