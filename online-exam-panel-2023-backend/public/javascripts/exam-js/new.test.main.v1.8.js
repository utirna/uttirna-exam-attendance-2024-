...
var Mytest = function () {
	...
	(this._printTypeOne = function (question, index_id) {
		...
		(this.setOptionData = function (option_text, option_image, index_id, btn_no, value, ans) {
			...
			if (option_image == '0') {
				...
				_tr =
					...
					value +
					'">' +
					'<label style="font-size: 15px;width:100%"><input class="avoid_click" ' +
					_checked +
		...
		(this.saveTest = function (data, callback) {
			$.ajax({
				url: _getUrl() + 'go?mac=00:00:00:00',
				type: 'post',
				data: data,
				success: function (data) {
					console.log(data, '-res data');
					if (data.call === 3) {
						var m1 = 'Access error, please contact Staff.';
		...
		if (test_question[current_question_index].student_ans != '') {
			...
			switch (quick_ans_ckeck) {
				case 'e':
					...
					break;
				case 'd':
					$('#i-opt-4').prop('checked', true);
					break;
				case 'c':
					$('#i-opt-3').prop('checked', true);
					break;
				case 'b':
					$('#i-opt-2').prop('checked', true);
					break;
				default:
					$('#i-opt-1').prop('checked', true);
					break;
			}
		}
	});
					...
					if (typeof scroll_pos != 'undefined') {
						if (scroll_pos > 160) {
							$('#question-btn').scrollTop(
								$('#question-btn').scrollTop() + $('#i-btn-view-' + (current_question_index + 1)).position().top - $('#question-btn').height() / 1.3
							);
						}
					}
