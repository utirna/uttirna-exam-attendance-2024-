...
$(function () {
	...
	$('#get-exam-list').on('click', function () {
		...
		$.ajax({
			url: getUrl() + 'get-exam-list',
			method: 'POST',
			data: { exam_list: JSON.stringify(test_id_list) },
			success: (data) => {
				removeLoading();
				if (data.call == 1) {
					LoadExamList(data.data, '#new_exams_list_tbody', 2);
				} else {
					if (data.call == 999) {
		...
		// $.post(getUrl() + 'get-exam-list', { exam_list: JSON.stringify(test_id_list) }, function (data, status) {
		// 	if (status == 'success') {
...
function LoadExamList(list, table_name, type) {
	...
	if (list.length == 0) {
		...
		// $('#get-exam-list').show()
		$(table_name).html('<tr><td colspan="10" style="text-align:center">No Exams Found1</td></tr>');
	}
	...
	if (list.length > 0) {
		...
		list.forEach(function (value, index) {
			...
			if (type == 1) {
				...
				// for attendance start status

				if (value.is_attendance_started == 1) {
					_tr += `
						<td class="text-center">
							<button 
								class='btn btn-danger btn-sm attendance-active-btn'
					...
					_tr += `
						<td class="text-center">
							<button class='btn btn-primary btn-sm attendance-active-btn'
								data-examDate="${value.exam_date}"
								data-exam-id="${value.id}"
								data-batch-id="${value.tm_allow_to}"
								data-type="start"
			...
			// for type 2 unsaved exam list
				...
				var isExamDownloaded = value.is_exam_downloaded == 1 ? true : false
				var isStudentsDownloaded =
					value.is_students_downloaded == 1 ? true : false
				var isPhotoSignDownloaded =
					value.is_sign_downloaded == 1 && value.is_photos_downloaded == 1
						? true
						: false
				var isFinalPublished = value.is_final_published == 1 ? true : false
				// prettier-ignore
				_tr += `<td class="text-center">
							<button type="button" 
									class="btn ${isExamDownloaded ? 'btn-success' : 'btn-primary'}  btn-sm download-exam-btn" 
									data-exam-id='${value.id}'
...
function isConfirm({ l = 'Confirm', t = '' }, cb) {
	showModal('#confirmModal');
	$('#confirmModalLabel').text(l);
	$('#confirm-modal-text').text(t);
...
$(document).on('click', '.download-exam-btn', function (e) {
	e.preventDefault();
...
async function downloadNodeDetails(center_code) {
	addLoading();
	$.ajax({
		url: getUrl() + 'download-node-details',
		type: 'POST',
		data: {
			center_code,
		},
		...
		error: function (error) {
			removeLoading();
			if (error.responseJSON.call == 999) {
...
function activeSession(pub_id) {
	$.post(getUrl() + '/startExamSession', { pub_id: pub_id }, function (data, status) {
		if (status == 'success') {
			var _json = data;
			switch (_json.call) {
				case 0:
...
let newExam = {
	...
	_submitExamDetails() {
		...
		$.post(getUrl() + '/loadExam', { pub_id: newExam.examId }, function (data, status) {
			if (status == 'success') {
				var _json = data;
				switch (_json.call) {
					case 0:
...
$(document).on('click', '.attendance-active-btn', activateAttendance);

function activateAttendance(e) {
	e.preventDefault();
	...
	addLoading();
	$.ajax({
		url: getUrl() + 'activate-attendance',
		method: 'POST',
		data: { examId, batchId, examDate, type },
		success: (data) => {
			removeLoading();
			if (data.call == 1) {
				window.location.reload();
...
$(document).on('click', '#examCompletedConfirmAuthPass', newExam._takeExamBackup);
...
function markStudentAsAbsent() {
	...
	addLoading();
	$.post(getUrl() + 'marked-as-absent', { pub_id: id, batch_no: batch_no }, function (data, status) {
...
function printStudentsDetails(list) {
	...
	if (list.length == 0) {
		...
		_html = list.map((el, i) => {
			...
			return ` <tr>
						...
						<td width='5%'> 
							<img class='h-100 w-100'  src='${is_mock_mode ? `/pics/photo_demo.jpg` : `/pics/_images/${el.sl_image}`}' alt='student-profile-img'/>
						</td>
						<td width='5%'> 
							<img class='h-100 w-100'  src='${is_mock_mode ? `/pics/sign_demo.jpg` : `/pics/_sign/${el.sl_sign}`}' alt='student-profile-img'/>
						</td>
						<td> ${el.sl_f_name} ${el.sl_m_name} ${el.sl_l_name} </td>
						<td> ${el.sl_contact_number} </td>
						<td> ${el.sl_application_number} </td>
						<td> ${el.sl_post} </td>
						<td> ${el.sl_password} </td>
					</tr>
				`
		});
	}
	...
	var _studentsListTable = new DataTable('.students-list-table', {
		destroy: true,
		responsive: true,
	});
}
