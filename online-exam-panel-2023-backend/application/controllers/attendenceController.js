...
var attendenceController = {
    ...
    getStudentDetails: async (req, res) => {
        try {
            ...
            let _studAttendenceCount = await attendenceModel.studentAttendanceCount(res.pool);

            if (student && student.sl_present_status == 1) {
                let _pcDetails = await attendenceModel.getStudentPcDetails(res.pool, id);
                return res.status(200).json({
                    success: true,
                    status: 200,
                    data: {
        ...
        try {
            ...
            studentPhoto.mv(destDir, async (err, result) => {
                if (err) {
                    ...
                    const _markAttendenceResult = await attendenceModel.markAttendencePresent(
                        res.pool,
                        data
                    );

                    let _attendanceCount = await attendenceModel.studentAttendanceCount(res.pool);
